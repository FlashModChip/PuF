...
public class LevelGenerator {
	...
	private int roomNumber ,roomTileSizeX,roomTileSizeY;   

    // parameters for random room generation (without templates)
    private int enemyNumber, enemyThreshold, itemNumber, itemThreshold;    
     ...
     * @param itemtresh
     */
    public LevelGenerator(int roomnum, int roomtilex, int roomtiley, int enemynum, int enemythresh, int itemnum,
            int itemtresh) {
        roomNumber = roomnum;
    ...
    /**
     * If you are wondering how this algorithm works : just like snakes =======(8)>
     * 
     * @param lvlnum
    ...
    public LevelData generateLevel(int lvlnum, boolean fromFile) {
        ...
        for (int i = 0; i < roomNumber - 1; i++) {
            ...
            case NORTH:
                // if room has an adjacent (north) room and there is no other room create new
                // room and connect them
                if ((new UUID(0, 0).equals(roomPointer.getadjacentRoom(Direction.NORTH)))
                        && (roomGrid[roomCoord.x - 1][roomCoord.x] == null)) {
                    tmp = new RoomData();
            ...
            case SOUTH:
                if ((new UUID(0, 0).equals(roomPointer.getadjacentRoom(Direction.NORTH)))
                        && (roomGrid[roomCoord.x + 1][roomCoord.y] == null)) {
                    tmp = new RoomData();
            ...
            case WEST:
                if ((new UUID(0, 0).equals(roomPointer.getadjacentRoom(Direction.WEST)))
                        && (roomGrid[roomCoord.x][roomCoord.y - 1] == null)) {
                    tmp = new RoomData();
            ...
            case EAST:
                if ((new UUID(0, 0).equals(roomPointer.getadjacentRoom(Direction.EAST)))
                        && (roomGrid[roomCoord.x][roomCoord.y + 1] == null)) {
                    tmp = new RoomData();
        ...
        return new LevelData(roomList, roomGrid, tmp, lvlnum, this.roomTileSizeX, this.roomTileSizeY);
    }
            ...
            for (int j = 0; j < roomdata[0].length; j++) {
                ...
                if (isWall(i, j, roomdata)) {
                    ...
                    if (isDoorPosition(i, j, roomdata)) {
                        ...
                        if ((!room.getadjacentRoom(Direction.WEST).equals(new UUID(0, 0))) && (i == 0)) {
                            roomdata[i][j] = TileCode.door;
                        } else if ((!room.getadjacentRoom(Direction.EAST).equals(new UUID(0, 0)))
                                && (i == roomdata.length - 1)) {
                            roomdata[i][j] = TileCode.door;
                        } else if ((!room.getadjacentRoom(Direction.NORTH).equals(new UUID(0, 0))) && (j == 0)) {
                            roomdata[i][j] = TileCode.door;
                        } else if ((!room.getadjacentRoom(Direction.SOUTH).equals(new UUID(0, 0)))
                                && (j == roomdata[0].length - 1)) {
                            roomdata[i][j] = TileCode.door;
                        } else {
    ...
    private int getRoomDataSingleTile(int x, int y, UUID[][] grid, HashMap<UUID, RoomData> roomlist) {
        ...
        return roomlist.get(grid[roomCoordX][roomCoordY]).getRoomData()[x - (roomCoordX * roomTileSizeX)][y
                - (roomCoordY * roomTileSizeY)];
    ...
    private void setRoomDataSingleTile(int x, int y, int val, UUID[][] grid, HashMap<UUID, RoomData> roomlist) {
        ...
        roomlist.get(grid[roomCoordX][roomCoordY]).getRoomData()[x - (roomCoordX * roomTileSizeX)][y
                - (roomCoordY * roomTileSizeY)] = val;
    ...
    public static void print2D(UUID[][] roomGrid, HashMap<UUID, RoomData> roomlist, int[][] largearr) {
        ...
        for (Map.Entry<UUID, RoomData> entry : roomlist.entrySet()) {
            System.out.println("ID:" + entry.getKey().toString().substring(0, 4) + " " + "North:"
                    + entry.getValue().getadjacentRoom(Direction.NORTH).toString().substring(0, 4) + " " + "South:"
                    + entry.getValue().getadjacentRoom(Direction.SOUTH).toString().substring(0, 4) + " " + "West:"
                    + entry.getValue().getadjacentRoom(Direction.WEST).toString().substring(0, 4) + " " + "East:"
                    + entry.getValue().getadjacentRoom(Direction.EAST).toString().substring(0, 4));
        }
