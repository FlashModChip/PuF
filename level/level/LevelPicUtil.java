...
public class LevelPicUtil
{
    ...
    {       
       ...
       for (int x = 0; x<imgWidth; x++) 
       {
           for (int y = 0; y<imgHeight; y++) 
           {
               ...
               else if (c.equals(TileCode.groundRGB)) 
               {
            	   tmp[x][y] = TileCode.ground;
               }              
               else if (c.equals(TileCode.itemRGB)) 
               {
            	   tmp[x][y] = TileCode.item;
               }
               else if (c.equals(TileCode.playerspawnRGB)) 
               {
            	   tmp[x][y] = TileCode.playerspawn;
               }
               else if (c.equals(TileCode.enemyRGB)) 
               {
            	   tmp[x][y] = TileCode.enemy;
    ...
    /**
     * Generates random integer between 2 values, because c# can do that and java can't XD
     * @param low
     * @param high
     * @return
     */
